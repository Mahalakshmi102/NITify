...
const { 
  ...
  getAdvisorAttendanceHistory,
  updateAdvisorAttendanceHistory,
  getWorkloads,
  createWorkload,
  updateWorkload,
  deleteWorkload
...
const authCoreOrAdvisor = (req, res, next) => {
  if (req.user && req.user.role === 'Faculty' && req.user.classAdvisorDetails?.isClassAdvisor) {
    return next();
  }
  return authCore(req, res, next);
};
...
router.get('/analytics/attendance-monitoring', authCoreOrAdvisor, getAttendanceMonitoringData);
router.get('/attendance/history-summary', authCoreOrAdvisor, getAdminAttendanceHistorySummary);
router.post('/attendance/bulk', authCoreOrAdvisor, saveBulkAttendance);
...
// Advisor Attendance History (Date-wise View & Edit)
router.get('/advisor/attendance-history', authCoreOrAdvisor, getAdvisorAttendanceHistory);
router.put('/advisor/attendance-history/:id', authCoreOrAdvisor, updateAdvisorAttendanceHistory);
