...
exports.protect = (req, res, next) => {
  ...
  try {
    ...
    // Parent tokens use student id but role is Parent — skip DB role overwrite
    if (req.user.role === 'Parent') {
      req.user.studentId = req.user.studentId || req.user.id;
      return next();
    }
...
exports.authorize = (...roles) => {
  return (req, res, next) => {
    ...
    const isAuthorized = roles.includes(userRole) || 
      ...
      (userRole === 'Parent' && roles.includes('Parent'));
