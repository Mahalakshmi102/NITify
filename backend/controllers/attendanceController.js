...
// Helper to get or create default Subject for daily attendance
const getDefaultSubject = async () => {
  try {
    ...
    console.error('Error in getDefaultSubject:', err);
    return null;
  }
};
...
exports.lockSession = async (req, res) => {
  try {
    const { sessionId, lock } = req.body;
    const shouldLock = lock !== undefined ? lock : true;
    ...
    // Authorization Check: Admin has access to all; HoD has access to department sessions; Faculty owns session.
    if (req.user.role === 'Admin') {
      // Allowed
    } else if (req.user.role === 'HoD') {
      // Look up subject of the session to match department
      const subject = await Subject.findById(session.subject);
      if (!subject || subject.department !== req.user.department) {
        return res.status(403).json({ message: 'Not authorized: Session is outside your department.' });
      }
    } else {
      if (session.faculty.toString() !== req.user.id) {
        return res.status(403).json({ message: 'Not authorized to lock/unlock this session.' });
      }
    }
    ...
    await Attendance.updateMany({ session: sessionId }, { $set: { locked: shouldLock } });
    
    res.json({ message: `Session ${shouldLock ? 'locked' : 'unlocked'} successfully.`, session });
  } catch (error) {
...
// Parent view: student attendance percentage and days present out of total working days
exports.getParentStudentAttendance = async (req, res) => {
  try {
    ...
    const student = await User.findById(studentId).select('name registerNumber rollNumber department year semester section');
    if (!student) {
    ...
    let attended = (counts['Present'] || 0) + (counts['Late'] || 0) + (counts['On-Duty'] || 0) + (counts['On Duty'] || 0);
    let conducted = attended + (counts['Absent'] || 0);
    ...
    const attendancePercentage = conducted > 0 ? Math.round((attended / conducted) * 100) : 0;
    const daysPresent = attended;
    const totalWorkingDays = conducted;
    ...
    res.json({
      ...
      daysPresent,
      totalWorkingDays,
      attendanceThreshold: policies.attendanceThreshold || 75
    });
  } catch (err) {
    console.error('getParentStudentAttendance error:', err);
    res.status(500).json({ message: 'Server Error' });
  }
};
...
exports.startSession = async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      await existing.save();
      return res.json({ message: 'Daily attendance session unlocked/restarted.', session: existing });
    }
    ...
    const session = new Session({
      ...
      faculty: req.user.id,
      date: startOfDay, // Midnight normalized
      period: 'Day', // Always 'Day'
      department: timetable.department,
      year: timetable.year,
      semester: timetable.semester,
      section: timetable.section,
    ...
    await session.save();
    res.status(201).json({ message: 'Daily attendance session created successfully.', session });
  } catch (error) {
...
exports.startCustomSession = async (req, res) => {
  try {
    let { subject, department, year, semester, section, date, period } = req.body;
    
    if (!department || !year || !semester || !section) {
      return res.status(400).json({ message: 'Class enrollment fields (department, year, semester, section) are required.' });
    }
    ...
    // Find default subject if none provided
    let finalSubject = subject;
    if (!finalSubject) {
      const defaultSub = await getDefaultSubject();
      if (defaultSub) {
        finalSubject = defaultSub._id;
      }
    }
    ...
    const session = new Session({
      ...
      semester,
      section,
      period: 'Day', // Always 'Day' for daily once
      qrToken,
      expiresAt
    });
    await session.save();
    
    // Fetch newly created session with populated subject if possible
    const populatedSession = await Session.findById(session._id).populate('subject');
    res.status(201).json({ message: 'Daily attendance session created successfully.', session: populatedSession });
  } catch (error) {
...
const computePendingList = async (facultyId) => {
  ...
  const todayPassedNoSession = todayTimetable.filter(slot => {
    const session = todaySessions.find(s => 
      ...
      String(s.year) === String(slot.year) &&
      String(s.semester) === String(slot.semester) &&
      String(s.section) === String(slot.section)
    );
  ...
  const pastSessions = await Session.find({
    faculty: facultyId,
    date: { $lt: todayStart }
  }).populate('timetable').lean();

  const hasPastSessionKeys = new Set();
  pastSessions.forEach(s => {
    const dateStr = new Date(s.date).toDateString();
    const dept = s.department || (s.timetable && s.timetable.department) || 'General';
    const yr = s.year || (s.timetable && s.timetable.year) || '1';
    const sem = s.semester || (s.timetable && s.timetable.semester) || '1';
    const sec = s.section || (s.timetable && s.timetable.section) || 'A';
    hasPastSessionKeys.add(`${dateStr}_${dept}_${yr}_${sem}_${sec}`);
  });
  ...
  for (const wDay of pastWorkingDays) {
    ...
    for (const slot of slots) {
      ...
      const classKey = `${dateStr}_${dept}_${yr}_${sem}_${sec}`;

      if (!hasPastSessionKeys.has(classKey)) {
        const className = `${dept} Y${yr} Sem ${sem} Sec ${sec}`;
        const alreadyAdded = pastMissedSlots.some(p => p.class === className && new Date(p.date).toDateString() === dateStr);
        if (!alreadyAdded) {
