...
exports.updateUser = async (req, res) => {
  try {
    ...
    if (req.user.role === 'Faculty') {
      ...
      // Restrict editing class details (dept) for Faculty
      if (department !== undefined && department !== existingUser.department) {
        return res.status(403).json({ message: 'Not authorized: Class Advisors cannot change student department.' });
      }
    }
...
exports.getAnalyticsOverview = async (req, res) => {
  try {
    ...
    const [studentCount, facultyCount, subjectCount, attendanceCount] = await Promise.all([
      ...
      User.countDocuments({ role: 'Faculty', ...userQuery }),
    ...
    for (const cls of todaysTimetable) {
      const session = todaysSessions.find(s => 
        ...
        String(s.semester) === String(cls.semester) &&
    ...
    const sessionsCreatedToday = todaysSessions.map(s => {
      ...
      return {
        ...
        year: cls ? cls.year : (s.year || 'Unknown'),
    ...
    if (req.user.role === 'HoD') {
      ...
      for (const y of yearsList) {
        const yearStudents = await User.find({ role: 'Student', department: req.user.department, year: y }).select('_id');
        const yStudentIds = yearStudents.map(s => s._id);
        const yAttendanceRecords = await Attendance.find({ student: { $in: yStudentIds } }).select('status').lean();

        let percentage = 0;
        if (yAttendanceRecords.length > 0) {
          const counts = { Present: 0, Late: 0, Absent: 0, 'On-Duty': 0, 'On Duty': 0, 'Medical Leave': 0, 'Casual Leave': 0 };
          yAttendanceRecords.forEach(rec => {
            counts[rec.status] = (counts[rec.status] || 0) + 1;
          });
          percentage = calculatePercentage(counts, policies);
        } else {
    ...
    res.json({
      ...
      departmentPerformance: req.user.role === 'HoD' ? yearWise : departmentWise,
...
exports.saveBulkAttendance = async (req, res) => {
  try {
    ...
    // Security check: if user is Faculty and Class Advisor, verify that all studentIds belong to their class
    if (req.user.role === 'Faculty' && req.user.classAdvisorDetails?.isClassAdvisor) {
      const adv = req.user.classAdvisorDetails;
      const invalidStudents = students.filter(s => 
        s.department !== adv.department ||
        String(s.year) !== String(adv.year) ||
        String(s.semester) !== String(adv.semester) ||
        s.section !== adv.section
      );
      if (invalidStudents.length > 0) {
        return res.status(403).json({ message: 'Not authorized: You can only submit attendance for your assigned class.' });
      }
    }
    ...
    if (!finalSubjectId) {
      let subject = await Subject.findOne({ code: 'DAY' });
      if (!subject) {
        subject = new Subject({
          name: 'Day Attendance',
          code: 'DAY',
    ...
    if (firstStudent) {
      sessionQuery.department = firstStudent.department;
      sessionQuery.year = String(firstStudent.year);
      sessionQuery.semester = String(firstStudent.semester);
      sessionQuery.section = firstStudent.section;
    } else {
      sessionQuery.subject = finalSubjectId;
      sessionQuery.period = 'Day';
    }
    ...
    if (!session) {
      ...
      const timetableEntry = await Timetable.findOne({ subject: finalSubjectId });
      ...
      session = new Session({
        ...
        expiresAt: new Date(startOfDay.getTime() + 24 * 60 * 60 * 1000) // +24 hours
      });
      await session.save();
    }
    ...
    await Log.create({
      ...
      details: `Bulk marked attendance for ${records.length} students on ${new Date(date).toLocaleDateString()} for subject ${finalSubjectId}`
    });
...
exports.handleBulkUpload = async (req, res) => {
  try {
    ...
    for (const name of workbook.SheetNames) {
      ...
      const parsed = parseSheetToJSON(sheet);
      if (parsed.length > 0) {
        data = parsed;
        sheetName = name;
        
        // 1. Detect role from sheet name
        const nameLower = name.toLowerCase();
        if (nameLower.includes('advisor') || nameLower.includes('adviser')) {
          detectedRole = 'Class Advisor';
        } else if (nameLower.includes('hod') || nameLower.includes('head of department') || nameLower.includes('head of the department')) {
          detectedRole = 'HoD';
        } else if (nameLower.includes('student')) {
          detectedRole = 'Student';
        }
        
        // 2. Detect role from cells before header row
        if (!detectedRole) {
          const rows = xlsx.utils.sheet_to_json(sheet, { header: 1, defval: '' });
            ...
            if (Array.isArray(r)) {
              for (const val of r) {
                if (val && typeof val === 'string') {
                  ...
                  if (valLower === 'class advisor' || valLower === 'class adviser') {
                    detectedRole = 'Class Advisor';
                    break;
                  } else if (valLower === 'hod' || valLower === 'head of department' || valLower === 'head of the department') {
                    detectedRole = 'HoD';
                    break;
                  } else if (valLower === 'student' || valLower === 'students') {
                    detectedRole = 'Student';
                    break;
                  }
                }
    ...
    if (type === 'users') {
      ...
      for (const row of data) {
        ...
        const registerNumberVal = getRowValue(row, ['registerNumber', 'registerNo', 'regNo', 'ber', 'facultyId', 'employeeId']);
        let registerNumber = registerNumberVal ? String(registerNumberVal).trim() : undefined;
        
        const nameVal = getRowValue(row, ['name', 'studentName', 'facultyName']);
        const name = nameVal ? String(nameVal).trim() : undefined;
        
        const rawEmail = getRowValue(row, ['email', 'emailId', 'emailAddress', 'mailId', 'mail_id', 'mail']);

        // Determine userRole dynamically
        let userRole = getRowValue(row, ['role']);
        if (!userRole) {
          if (detectedRole) {
            userRole = detectedRole;
          } else {
            // Check headers for clues
            const hasParentNo = getRowValue(row, ['parentNo', 'parentMobile', 'parentMobileNo', 'parentNo', 'guardianMobile', 'parentno', 'parent_no', 'parent_mobile']) !== undefined;
            const hasRegNo = getRowValue(row, ['registerNumber', 'registerNo', 'regNo', 'ber', 'facultyId']) !== undefined;
            if (hasParentNo || hasRegNo) {
              userRole = 'Student';
            } else {
          ...
          } else if (normalizedRole === 'hod' || normalizedRole === 'head of department' || normalizedRole === 'head of the department') {
            userRole = 'HoD';
          } else {
        ...
        if (name) {
          let email = rawEmail ? String(rawEmail).trim() : undefined;
          if (!email) {
            email = registerNumber 
              ? `${registerNumber}@nit.edu` 
              : `${String(name).toLowerCase().replace(/\s+/g, '')}@nit.edu`;
          }

          if (!registerNumber && (userRole === 'Class Advisor' || userRole === 'HoD' || userRole === 'Faculty')) {
          ...
          const mobile = getRowValue(row, ['mobileNo', 'mobile', 'mobileNumber', 'studentMobile', 'mobileno', 'mobile_no']);
          const parentMobile = getRowValue(row, ['no', 'parentMobile', 'parentMobileNo', 'parentNo', 'guardianMobile', 'parentno', 'parent_no', 'parent_mobile']);
          ...
          if (studentUser) {
            ...
            if (userRole === 'Class Advisor') {
              studentUser.classAdvisorDetails = {
                isClassAdvisor: true,
            ...
            // Create new user
            const rawPassword = getRowValue(row, ['password']) || ((userRole === 'Student' || userRole === 'Faculty' || userRole === 'Class Advisor' || userRole === 'HoD') && dobString ? dobString : 'password123');
            ...
            await User.create({ 
               ...
               designation: getRowValue(row, ['designation']) ? String(getRowValue(row, ['designation'])).trim() : (userRole === 'Class Advisor' ? 'Class Advisor' : (userRole === 'HoD' ? 'Professor & Head (HOD)' : undefined)),
               ...
               classAdvisorDetails: userRole === 'Class Advisor' ? {
                 isClassAdvisor: true,
                 department: department,
                 year: year || '1',
                 semester: semester || '1',
               ...
               parentDetails: userRole === 'Student' ? {
            ...
            try {
              ...
              console.error('Mail sending skipped or failed during bulk upload:', mailErr.message);
            }
...
exports.getAttendanceMonitoringData = async (req, res) => {
  try {
    ...
    const isClassAdvisor = req.user.classAdvisorDetails && req.user.classAdvisorDetails.isClassAdvisor;
    let activeDept = req.user.role === 'HoD' ? req.user.department : department;
    let activeYear = year;
    ...
    if (isClassAdvisor && req.user.role === 'Faculty') {
      const adv = req.user.classAdvisorDetails;
      activeDept = adv.department;
      activeYear = adv.year;
      activeSemester = adv.semester;
      activeSection = adv.section;
    }

    let userQuery = { role: 'Student' };
    if (activeDept) userQuery.department = activeDept;
    if (activeYear) userQuery.year = String(activeYear);
    if (activeSemester) userQuery.semester = String(activeSemester);
    if (activeSection) userQuery.section = String(activeSection);

    const students = await User.find(userQuery).select('_id');
    const studentIds = students.map(s => s._id);

    let sessionMatch = { date: { $gte: targetDate, $lt: nextDay } };
    if (activeDept) sessionMatch.department = activeDept;
    if (activeYear) sessionMatch.year = String(activeYear);
    if (activeSemester) sessionMatch.semester = String(activeSemester);
    if (activeSection) sessionMatch.section = String(activeSection);
    if (period) sessionMatch.period = period;
  ...
  try {
    ...
    holidays.forEach(h => {
      ...
      if (!dailyTimelineMap[dateStr]) {
        dailyTimelineMap[dateStr] = { date: dateStr, status: 'Holiday', recordId: null };
      }
    });
...
exports.getAdvisorStats = async (req, res) => {
  try {
    ...
    const students = await User.find({
      ...
      section: adv.section
    }).select('_id name registerNumber rollNumber email mobile parentDetails batch section year semester').lean();
    ...
    // 3. Defaulters (< 75%), At-Risk (75% to 80%) and Full Students list
    const defaulters = [];
    const atRisk = [];
...
exports.updateAdvisorRecord = async (req, res) => {
  try {
    ...
    if (req.user.role === 'Faculty' && existingRecord.advisor.toString() !== req.user.id) {
...
exports.deleteAdvisorRecord = async (req, res) => {
  try {
    ...
    if (req.user.role === 'Faculty' && record.advisor.toString() !== req.user.id) {
...
// --- Class Advisor Attendance History (Date-wise View & Edit) ---
exports.getAdvisorAttendanceHistory = async (req, res) => {
  try {
    ...
    if (isClassAdvisor && req.user.role === 'Faculty') {
      ...
      classFilter = {
        department: adv.department,
        year: String(adv.year),
        semester: String(adv.semester),
        section: adv.section
      };
    } else if (req.user.role === 'HoD') {
      classFilter.department = req.user.department;
      if (req.query.year) classFilter.year = String(req.query.year);
      if (req.query.semester) classFilter.semester = String(req.query.semester);
      if (req.query.section) classFilter.section = req.query.section;
    } else if (['Admin', 'Principal', 'CoE'].includes(req.user.role)) {
      if (req.query.department) classFilter.department = req.query.department;
      if (req.query.year) classFilter.year = String(req.query.year);
      if (req.query.semester) classFilter.semester = String(req.query.semester);
      if (req.query.section) classFilter.section = req.query.section;
    } else {
    ...
    // Find sessions for this date and class
    const sessionQuery = {
      date: { $gte: targetDate, $lt: nextDay },
      ...classFilter
    };
    ...
    if (sessionIds.length === 0) {
      return res.json({ date, records: [], summary: { total: 0, present: 0, absent: 0, late: 0, onDuty: 0 } });
    }

    // Fetch attendance records for those sessions
    const records = await Attendance.find({ session: { $in: sessionIds } })
      .populate('student', 'name registerNumber rollNumber department year semester section')
      .sort({ 'student.registerNumber': 1 })
      .lean();

    // Compute summary
    let presentCount = 0, absentCount = 0, lateCount = 0, onDutyCount = 0;
    const formattedRecords = records.map(r => {
      if (r.status === 'Present') presentCount++;
      else if (r.status === 'Absent') absentCount++;
      else if (r.status === 'Late') lateCount++;
      else if (['On-Duty', 'On Duty'].includes(r.status)) onDutyCount++;

      return {
        _id: r._id,
        studentId: r.student?._id,
        studentName: r.student?.name || 'Unknown',
        registerNumber: r.student?.registerNumber || r.student?.rollNumber || '-',
        status: r.status,
        remarks: r.remarks || '',
        markedAt: r.markedAt,
        markedBy: r.markedBy,
    ...
    formattedRecords.sort((a, b) => (a.registerNumber || '').localeCompare(b.registerNumber || ''));
    ...
    console.error('getAdvisorAttendanceHistory error:', error);
    res.status(500).json({ message: 'Error fetching attendance history.' });
  }
};

exports.updateAdvisorAttendanceHistory = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const validStatuses = ['Present', 'Absent', 'Late', 'On-Duty', 'On Duty', 'Medical Leave', 'Casual Leave'];
    if (!validStatuses.includes(status)) {
      return res.status(400).json({ message: 'Invalid status value.' });
    }

    const record = await Attendance.findById(id).populate('student', 'name registerNumber department year semester section');
    if (!record) {
      return res.status(404).json({ message: 'Attendance record not found.' });
    }

    // Security: verify the record belongs to the advisor's class
    const isClassAdvisor = req.user.classAdvisorDetails && req.user.classAdvisorDetails.isClassAdvisor;
    if (isClassAdvisor && req.user.role === 'Faculty') {
      const adv = req.user.classAdvisorDetails;
      ...
      if (
        ...
        return res.status(403).json({ message: 'Not authorized: Student is not in your assigned class.' });
      }
    }
    ...
    // Audit Log
    await createLog('Edit Student Attendance (Advisor History)', req.user, 'Attendance', record._id, {
      oldValue: oldStatus,
      newValue: status,
      reason: remarks || 'Advisor attendance history correction',
      targetDept: record.student?.department || 'General',
      targetSemester: record.student?.semester,
      targetSection: record.student?.section,
      student: record.student?._id || record.student,
      details: `Advisor updated attendance for ${record.student?.name || 'student'} from ${oldStatus} to ${status}`
    });
    ...
    console.error('updateAdvisorAttendanceHistory error:', error);
    res.status(500).json({ message: 'Error updating attendance record.' });
  }
};
