...
export default function StudentsManage({ departmentOnly, isReadOnly = false }) {
  ...
  const handleExport = () => {
    const csvRows = [
      ['S.No', 'RegisterNo.', 'Student Name', 'Batch', 'Year', 'Semester', 'Section', 'Mobile No.', 'Parent Mobile', 'Department', 'Status']
    ];
  ...
  return (
    <div className="space-y-6 pb-10">
          ...
          {paginatedData.map((s) => (
            <div key={s._id} className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-xs font-semibold text-slate-500">
                ...
                <div>
                  <span className="block text-[9px] uppercase font-bold text-slate-400">Class (Year / Sem / Sec)</span>
                  <span className="text-slate-700">
                    {s.year ? `Y${s.year}` : '-'} / {s.semester ? `S${s.semester}` : '-'} / {s.section ? `Sec ${s.section}` : '-'}
                  </span>
                </div>
                <div>
              ...
              <div className="flex items-center justify-end gap-1.5 pt-2.5 border-t border-slate-100">
                <button 
                  onClick={() => setSelectedStudentId(s._id)}
                  className="p-1.5 border border-slate-200 rounded text-slate-505 hover:bg-slate-55 hover:text-indigo-600 transition"
          ...
          <table className="w-full text-center text-sm whitespace-nowrap">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="p-4 font-bold text-slate-800 border-r border-slate-100">S.No</th>
              ...
              {paginatedData.map((s, idx) => {
                ...
                return (
                  <tr key={s._id} className="hover:bg-slate-55 transition">
                    ...
                    <td className="p-4 font-semibold text-slate-700 border-r border-slate-100">{sNo}</td>
      ...
      {/* Bottom Cards Row */}
      <div className={`grid grid-cols-1 sm:grid-cols-2 ${isReadOnly ? 'md:grid-cols-3' : 'md:grid-cols-4'} gap-4 pt-4`}>
        <div 
          onClick={() => {
