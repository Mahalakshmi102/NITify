...
export default function AnalyticsDashboard({ departmentOnly, setActiveTab }) {
  ...
  // Year filters for HOD Dashboard panels
  const [livePeriodYearFilter, setLivePeriodYearFilter] = useState('All');
  const [sessionYearFilter, setSessionYearFilter] = useState('All');
  const [timetableYearFilter, setTimetableYearFilter] = useState('All');

  // Session toggling state & function
  const [togglingSessionId, setTogglingSessionId] = useState(null);
  const handleToggleSessionLock = async (sessionId, currentLockedStatus) => {
    try {
      setTogglingSessionId(sessionId);
      await axios.post(apiUrl('/api/attendance/lock'), {
        sessionId,
        lock: !currentLockedStatus
      ...
      console.error(err);
      alert('Failed to update session lock status.');
    } finally {
  ...
  const filteredPeriodClasses = livePeriod?.classes?.filter(c => {
    const matchesSearch = c.faculty.toLowerCase().includes(periodFilter.toLowerCase()) ||
    ...
    const matchesYear = livePeriodYearFilter === 'All' || String(c.year) === String(livePeriodYearFilter);
    return matchesSearch && matchesYear;
  }) || [];

  const filteredSessions = sessionEngine?.sessions?.filter(s => {
    return sessionYearFilter === 'All' || String(s.year) === String(sessionYearFilter);
  }) || [];

  const filteredTimetableClasses = livePeriod?.classes?.filter(slot => {
    return timetableYearFilter === 'All' || String(slot.year) === String(timetableYearFilter);
  }) || [];
    ...
    <div className="space-y-8 font-sans">
      ...
      {!departmentOnly ? (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.01)] flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-center mb-4 pb-4 border-b border-slate-55">
                <div>
                  <h3 className="font-extrabold text-slate-800 text-sm flex items-center gap-2">
                ...
                <table className="w-full text-left text-xs whitespace-nowrap">
                  <thead className="bg-slate-50 text-slate-500 font-bold sticky top-0">
                    <tr>
                      ...
                      <th className="p-3 text-center">Status</th>
                      {departmentOnly && <th className="p-3 text-right">Actions</th>}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                      ...
                      <tr key={i} className="hover:bg-slate-50/50 transition">
                        ...
                        {departmentOnly && (
                          <td className="p-3 text-right">
                            <button
                              disabled={togglingSessionId === s.id}
                              onClick={() => handleToggleSessionLock(s.id, s.isLocked)}
                            ...
                            >
                              {togglingSessionId === s.id ? 'Processing...' : (s.isLocked ? 'Unlock' : 'Lock')}
                            </button>
                          </td>
                        )}
                      </tr>
                    ...
                    {filteredSessions.length === 0 && (
                      <tr>
                        <td colSpan={departmentOnly ? "6" : "5"} className="p-6 text-center text-slate-400 italic">No attendance sessions created today.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
                      ...
                      <tr key={i} className="hover:bg-slate-50/50 transition">
                        ...
                        <td className="p-3 text-center">
                          <span className={`px-2 py-0.5 rounded text-[9px] font-extrabold ${
                            ...
                            {stu.attendance < 55 ? 'CRITICAL' : 'WARNING'}
                          </span>
                        </td>
                        <td className="p-3 text-right">
                          <button 
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.01)] flex flex-col justify-between">
          <div>
            <h3 className="font-extrabold text-slate-800 text-sm flex items-center gap-2 mb-4">
              ...
              {departmentOnly ? 'Year-wise Attendance Comparison' : 'Department Attendance Comparison'}
            </h3>
            <div className="h-[220px] w-full min-w-0 relative">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                <BarChart data={departmentPerformance || []} margin={{ top: 10, right: 0, left: -25, bottom: 0 }}>
                  ...
                  <XAxis dataKey={departmentOnly ? "year" : "department"} axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#64748b', fontWeight: 'bold' }} />
              ...
              <div className="p-6 max-h-[75vh] overflow-y-auto custom-scrollbar space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-5">
                    ...
                    <table className="w-full text-center text-xs border-collapse whitespace-nowrap">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {selectedStudentDetail.dailyTimeline?.map((day, idx) => {
                          let statusClass = 'bg-slate-50 text-slate-400 border border-slate-200';
                          ...
                          } else if (day.status === 'On-Duty' || day.status === 'On Duty') {
                            statusClass = 'bg-indigo-50 text-indigo-700 border border-indigo-200';
                          } else if (day.status === 'Medical Leave' || day.status === 'Casual Leave') {
                          ...
                          return (
                            <tr key={idx} className="hover:bg-slate-50/50">
                              <td className="p-3 font-bold text-slate-700 text-left">
                                {new Date(day.date).toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' })}
                              </td>
                              <td className="p-3">
                                <span className={`inline-flex items-center justify-center px-2.5 py-1 rounded-full font-black text-[9px] uppercase tracking-wider min-w-[90px] ${statusClass}`}>
                                  {day.status}
                                </span>
                              </td>
                              <td className="p-3 text-slate-500 font-semibold">{day.markedBy || '-'}</td>
                              <td className="p-3 text-slate-500 font-semibold font-mono">{day.entryType || '-'}</td>
                            </tr>
                          );
                        })}
                        {(!selectedStudentDetail.dailyTimeline || selectedStudentDetail.dailyTimeline.length === 0) && (
                          <tr>
                            <td colSpan="4" className="p-8 text-center text-slate-400 font-medium italic">
                              No daily attendance logs found.
                            </td>
                          </tr>
                        )}
                      </tbody>
                  ...
                  <div className="flex flex-wrap gap-4 mt-3 text-xs text-slate-500 justify-center">
                    ...
                    <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-indigo-650"></span> On Duty</span>
                    <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-teal-500"></span> Leave</span>
                    <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-purple-600"></span> Holiday</span>
                  </div>
                </div>
                                ...
                                <td className="p-2 text-center">
                                  {editingRecordId === log._id ? (
                                    ...
                                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${
                                      ...
                                      (log.status === 'Medical Leave' || log.status === 'Casual Leave') ? 'bg-yellow-50 text-yellow-700' :
                                      (log.status === 'On-Duty' || log.status === 'On Duty') ? 'bg-blue-50 text-blue-700' :
