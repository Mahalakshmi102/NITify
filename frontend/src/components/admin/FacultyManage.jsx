...
export default function FacultyManage({ departmentOnly, advisorsOnly }) {
  const { user } = useAuth();
  ...
  const [advisorFilter, setAdvisorFilter] = useState(!!advisorsOnly);
  const [showMobileFilters, setShowMobileFilters] = useState(false);
  const isAdvisorOrHodView = advisorsOnly || advisorFilter || roleFilter === 'Class Advisor' || roleFilter === 'HoD';
  ...
  const handleExport = () => {
    let csvRows = [];
    if (isAdvisorOrHodView) {
      csvRows.push(['S.No', 'Department', 'Batch', 'Section', 'Name', 'Mail Id', 'Status']);
      filteredFaculty.forEach((f, idx) => {
        csvRows.push([
          idx + 1,
          f.department || '',
          f.classAdvisorDetails?.batch || f.batch || '',
          f.classAdvisorDetails?.section || f.section || '',
          f.name || '',
          f.email || '',
    ...
    const filePrefix = isAdvisorOrHodView ? 'advisor_hod_directory' : 'faculty_directory';
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-2">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-800">Faculty & Staff Management</h2>
        ...
        >
          <UserSquare2 className="w-5 h-5" /> Add Faculty Manually
        </button>
      </div>
          ...
          <div className="flex gap-3 w-full md:w-auto">
            ...
            {selectedIds.length > 0 && (
              <button 
                ...
                className="flex-1 md:flex-initial bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-6 rounded-lg flex items-center justify-center gap-2 transition shadow-md shadow-red-500/20 text-sm h-[38px]"
              >
      ...
      <div className="bg-white rounded-2xl shadow-[0_2px_12px_-4px_rgba(0,0,0,0.05)] border border-slate-100 overflow-hidden">
            ...
            <div key={f._id} className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-xs font-semibold text-slate-500">
                ...
                {isAdvisorOrHodView ? (
                  <>
                    <div>
                      <span className="block text-[9px] uppercase font-bold text-slate-400">Batch</span>
                      <span className="text-slate-700">{f.classAdvisorDetails?.batch || f.batch || '-'}</span>
                    </div>
                    <div>
                      <span className="block text-[9px] uppercase font-bold text-slate-400">Section</span>
                      <span className="text-slate-700">{f.classAdvisorDetails?.section || f.section || '-'}</span>
                    </div>
                    <div>
                      <span className="block text-[9px] uppercase font-bold text-slate-400">Mail Id</span>
                      <span className="text-slate-700 truncate block max-w-[130px] font-mono">{f.email || '-'}</span>
                    </div>
                  </>
                ) : (
                  <>
        ...
        {/* Desktop Table View */}
        <div className="hidden md:block overflow-x-auto">          <table className="w-full text-center text-sm whitespace-nowrap">
            <thead className="bg-slate-50 border-b border-slate-200">
              {isAdvisorOrHodView ? (
                <tr>
                  ...
                  <th className="p-4 font-bold text-slate-800 border-r border-slate-100 w-12">S.No</th>
                  <th className="p-4 font-bold text-slate-800 border-r border-slate-100 text-left">Department</th>
                  ...
                  <th className="p-4 font-bold text-slate-800 border-r border-slate-100 text-left">Academic Staff Name</th>
                  <th className="p-4 font-bold text-slate-800 border-r border-slate-100 text-left">Mail Id</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {paginatedData.map((f, idx) => (
                <tr key={f._id} className="hover:bg-slate-55 transition">
                  ...
                  {isAdvisorOrHodView ? (
                    <>
                      <td className="p-4 font-semibold text-slate-700 border-r border-slate-100 w-12">
                        {(currentPage - 1) * itemsPerPage + idx + 1}
                      </td>
                      <td className="p-4 font-semibold text-slate-650 border-r border-slate-100 text-left">
                        {f.department || '-'}
                      </td>
                      ...
                      <td className="p-4 text-slate-600 border-r border-slate-100 text-left font-mono">
                        {f.email || '-'}
                      </td>
                    </>
                  ) : (
                  ...
                  <td className="p-4 text-center">
                    <div className="flex justify-center items-center gap-1.5">
                      ...
                      <button 
                        onClick={() => handleResetToDob(f)}
                        className="p-1.5 border border-slate-200 rounded text-slate-500 hover:bg-slate-100 hover:text-purple-650 transition"
                        title="Reset Password to DOB"
                      >
                        <RotateCcw className="w-4 h-4" />
                      </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 pt-4">
        <div 
          ...
          className={`border p-4 flex items-center gap-4 cursor-pointer hover:-translate-y-1 hover:shadow-md transition duration-200 active:scale-98 rounded-2xl ${
            roleFilter === 'Faculty' && !advisorFilter
        ...
        >
          ...
          <div>
            <h4 className="text-sm font-bold text-slate-800">Faculty Profiles</h4>
            <p className="text-[11px] font-semibold text-slate-500">Filter profiles by Faculty role</p>
          </div>
        </div>
