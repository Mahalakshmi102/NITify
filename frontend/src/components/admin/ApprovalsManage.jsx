...
export default function ApprovalsManage({ departmentOnly }) {
  ...
  const fetchRequests = async () => {
    try {
      const url = departmentOnly
        ? apiUrl('/api/requests?approvalStage=HOD&status=Pending')
        : apiUrl('/api/requests');
  ...
  const pendingRequests = departmentOnly
    ? requests.filter(r => r.status === 'Pending' && r.targetModel === 'Leave' && r.approvalStage === 'HOD')
