...
export default function AttendanceMonitoring({ departmentOnly, advisorClass, defaultSubTab }) {
  const { user } = useAuth();
  ...
  const [filters, setFilters] = useState({
    department: advisorClass ? advisorClass.department : (departmentOnly ? user?.department || 'CSE' : 'CSE'),
    academicYear: advisorClass ? (
      advisorClass.year === '1' ? '2025-2026' : 
      advisorClass.year === '2' ? '2024-2025' : 
      advisorClass.year === '3' ? '2023-2024' : 
      '2022-2023'
    ) : '',
    year: advisorClass ? advisorClass.year : '',
    semester: advisorClass ? advisorClass.semester : '',
    section: advisorClass ? advisorClass.section : '',
    subjectId: '',
    facultyId: advisorClass ? user?._id || user?.id || '' : '',
    date: new Date().toISOString().split('T')[0],
    period: 'Day'
  });

  const [activeSubTab, setActiveSubTab] = useState(defaultSubTab || (user?.role === 'HoD' ? 'history' : 'mark')); // 'mark' or 'logs' or 'history'

  React.useEffect(() => {
    if (advisorClass && filters.department && filters.year && filters.semester && filters.section) {
      loadStudents();
    }
  }, [advisorClass]);
  ...
  const loadStudents = async () => {
    ...
    try {
      ...
      if (matchedClass && matchedClass.students && matchedClass.students.length > 0) {
        ...
        setMessage({ text: `Loaded existing attendance records for this date.`, type: 'success' });
      } else {
  ...
  const saveAttendance = async () => {
    ...
    // No subject check needed for day-wise
    setSaving(true);
    setMessage({ text: '', type: '' });
    try {
    ...
    <div className="space-y-6 pb-10">
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-[#f0f7ff] border border-blue-100 p-5 rounded-2xl flex items-center gap-4 shadow-sm">
                  ...
                  <div>
                    <span className="text-[10px] font-black text-slate-455 uppercase tracking-wide">Total Days Scheduled</span>
                    <p className="text-2xl font-black text-slate-800 mt-0.5">{historySummary?.stats?.totalHoursAssigned || 0}</p>
                    <p className="text-[10px] font-semibold text-slate-400">Calendar days</p>
                  </div>
                </div>
                ...
                <div className="bg-[#f0fdf4] border border-emerald-100 p-5 rounded-2xl flex items-center gap-4 shadow-sm">
                  ...
                  <div>
                    <span className="text-[10px] font-black text-slate-450 uppercase tracking-wide">
                      {advisorClass ? "Submitted Attendance Records" : "Attendance Completed"}
                    </span>
                ...
                <div className="bg-[#f0f9ff] border border-sky-100 p-5 rounded-2xl flex items-center gap-4 shadow-sm">
                  ...
                  <div>
                    <span className="text-[10px] font-black text-slate-455 uppercase tracking-wide">Total Days Completed</span>
                    <p className="text-2xl font-black text-slate-800 mt-0.5">{historySummary?.stats?.totalHoursFinished || 0}</p>
                    <p className="text-[10px] font-semibold text-sky-655">Conducted days</p>
                  </div>
                </div>
                ...
                <div className="bg-[#fff1f2] border border-rose-100 p-5 rounded-2xl flex items-center gap-4 shadow-sm">
                  ...
                  <div>
                    <span className="text-[10px] font-black text-slate-450 uppercase tracking-wide">
                      {advisorClass ? "Pending Attendance Submissions" : "Pending Attendance"}
                    </span>
              ...
              <div className="bg-white rounded-2xl shadow-[0_2px_12px_-4px_rgba(0,0,0,0.05)] border border-slate-100 overflow-hidden">
                <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                  ...
                  {user?.role === 'HoD' && (
                    <button 
                      onClick={fetchHistorySummary}
                      className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-xs uppercase transition shadow-md shadow-indigo-500/20 flex items-center gap-2 cursor-pointer"
                    >
                      <RotateCcw className="w-4 h-4" /> Refresh Data
                    </button>
                  )}
      ...
      <div className="bg-white p-6 rounded-2xl shadow-[0_2px_12px_-4px_rgba(0,0,0,0.05)] border border-slate-100 flex flex-col gap-5">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <div className="flex flex-col">
            <label className="text-[11px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Department</label>
            <select name="department" value={filters.department} onChange={handleFilterChange} disabled={departmentOnly || !!advisorClass} className="border border-slate-200 rounded-lg p-2.5 text-sm font-semibold text-slate-700 outline-none focus:border-blue-500 disabled:opacity-75 disabled:bg-slate-50">
              <option value="">All Departments</option>
              <option value="CSE">Computer Science & Engineering</option>
              <option value="ECE">Electronics & Communication</option>
              <option value="MECH">Mechanical Engineering</option>
          ...
          <div className="flex flex-col">
            ...
            <select name="academicYear" value={filters.academicYear || ''} onChange={handleFilterChange} disabled={!!advisorClass} className="border border-slate-200 rounded-lg p-2.5 text-sm font-semibold text-slate-700 outline-none focus:border-blue-500 disabled:opacity-75 disabled:bg-slate-50">
          ...
          <div className="flex flex-col">
            ...
            <select name="year" value={filters.year || ''} onChange={handleFilterChange} disabled={!!advisorClass} className="border border-slate-200 rounded-lg p-2.5 text-sm font-semibold text-slate-700 outline-none focus:border-blue-500 disabled:opacity-75 disabled:bg-slate-50">
          ...
          <div className="flex flex-col">
            ...
            <select name="semester" value={filters.semester} onChange={handleFilterChange} disabled={!!advisorClass} className="border border-slate-200 rounded-lg p-2.5 text-sm font-semibold text-slate-700 outline-none focus:border-blue-500 disabled:opacity-75 disabled:bg-slate-50">
          ...
          <div className="flex flex-col">
            ...
            <select name="section" value={filters.section} onChange={handleFilterChange} disabled={!!advisorClass} className="border border-slate-200 rounded-lg p-2.5 text-sm font-semibold text-slate-700 outline-none focus:border-blue-500 disabled:opacity-75 disabled:bg-slate-50">
              <option value="">All</option>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
          <div className="flex flex-col">
            ...
            <select name="facultyId" value={filters.facultyId} onChange={handleFilterChange} disabled={!!advisorClass} className="border border-slate-200 rounded-lg p-2.5 text-sm font-semibold text-slate-700 outline-none focus:border-blue-500 disabled:opacity-75 disabled:bg-slate-50">
          ...
          <button 
            onClick={() => {
              if (advisorClass) {
                ...
                setFilters({ ...filters, department: '', year: '', semester: '', section: '' });
              }
            }}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
        ...
        <div className="bg-[#eff6ff] border border-blue-100 p-4 rounded-2xl flex items-center gap-4">
          <div className="bg-blue-100 p-3 rounded-xl"><Clock className="w-6 h-6 text-blue-600" /></div>
          <div>
